...
- Q5 定案為「本次一併建立專案／系統階層」，使本 intent 從「加一層編排」擴為「同時建立新的核心資料模型」。此決定會觸發 project.md 的 blocking 規則（schema_rbac.sql 與 DEPLOY.md 必須同步），下游 feasibility 與 scope-definition 必須把它當成第一級範圍項而非附帶工作。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:67f2de7d8f29be84bb0c7f3a1bfcfc94dac414ff4a098afd3ddebb72d5245c04 -->
- 描述中的「專案 → 系統 → 架構圖」階層在 repo 中不存在，故不視為既有事實而改列為 Q5 的待決問題；`schema_rbac.sql` 只有 users→user_diagrams，backend 全樹 `system_id` 命中 0 次。若逕自當成既有階層，整份需求會建立在一個不存在的資料模型上。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:202f27f5bcef1942dd80ef08356392d9a73fefbe2b74254916ee4603747ac1c2 -->
- 描述點名的「成本及 FinOps agent」在 repo 中只有 `FinOps_Analyst` 這個 RBAC 角色，沒有任何成本資料、服務或模組，故 Q6 把「編排既有能力」與「同時建成本能力」拆成不同選項，讓範圍差異顯性化。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:6b71456e545377abeb0230607e159bf41ce4070ef02c9f74eefc71366ed7c54b -->
- 收齊答案後的矛盾偵測抓到兩處跨題不一致（Q2 的管理者 vs Q7 的共享範圍、Q2 的成本關注者 vs Q6 的成本空殼），依 stage-protocol §3 加開 Q10／Q11 當場定錨並回寫問題檔，未讓歧義流入下一階段。問題數因此由 9 增為 11。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:6de6fa66933fa67ed1dbac535180da6ff95275cffe2adfed36dbb06e493c270d -->
- 問題數為 9 題，略高於 Standard depth 的 5–8 題指引。理由是本 intent 橫跨編排框架、session 層、三種記憶、串流、推播與多意圖識別六個面向，且其中兩個面向（專案階層、成本能力）的前提在 repo 中不成立，壓到 8 題以內會使其中一項無法被問到。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:ef1dee52c6bd1edf83b02f15a717ccc07c801fc870742f549291f5145abd4900 -->
- Q11 定案為 B（空殼要能正確回「尚未提供」）而非 C（實作最小成本能力）。取捨是：保住「成本關注者」這個角色在第一版有可驗證的行為（路由正確、回覆明確），同時不把成本資料來源這個獨立子系統拉進本 intent——而本 intent 已因 Q5=B 承擔了新資料模型。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:63b0f3e4722dfa83d4d96a3b9978ce3e04f1ce5b2f2f2b3942def064da62f5f9 -->
- 提問時把某題的選項順序重排（為了讓建議項排前面），寫回問題檔時卻依**位置**而非依**內容**對應，把 F11 使用者實選的 A 誤記為 C。此即 `requirements-analysis:260822-ra-c2` 警告的形狀，但該規則談的是「選項數超過上限時先收斂」，未涵蓋「順序重排後的回寫對應」。已在問題檔就地記錄更正與原因。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:feasibility:7e354cfdf52d22ea0f79d38813303a79ef9c6307e99264d0830292629f092965 -->
- 抽象的衝突清單不足以讓人察覺答案讀反；**具體呈現「照此執行下一步會產出什麼」才有效**。本站先以矛盾偵測列出 S9 結果與 [Q1]／[Q3]／[Q5] 的三處衝突並明示「方向可能讀反」，使用者回覆「沒反，就是這樣」；直到 S10 把後果具體化為「兩項 Must 各自依賴一項 Should，故 Must 集合無法獨立交付」，使用者才主動更正為 8 Must／2 Should。下次遇到「答案與多處上游定案衝突且疑似讀反」時，直接推導到下一步的具體產出，不要停在衝突清單。 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:scope-definition:5016d9d84951a6438e6965d693e3350ff564d4439eeeb17f37daee3162571973 -->
- 當一個新決定製造出的張力，其形狀與本 stage 稍早已用「加開問題讓使用者定案」處理過的張力相同時，必須同樣加開問題，不得改由 conductor 自行寫一句化解句並掛 [Q<n>] 標籤。可執行檢查：每次為了消解跨題矛盾而在 artifact 內寫下「適用前提」「服務在 X 處完成」這類調和句時，先問「本 stage 有沒有處理過形狀一樣的張力？它當時是怎麼處理的？」；有先例就比照。本輪實例：Q10 為「管理者是服務對象 vs 管理頁不在共享範圍」加開一題定案，而成本關注者改為直接服務後產生的「成本關注者是服務對象 vs 成本頁不在共享範圍」是同一個形狀，卻被我用一句自行合成的推論句化解並掛上 [Q7]——逐字核對 Q7 與 Q12 的作答，兩題都沒有一個字支持那句話。既有的 intent-capture:c11 只管「掛標籤前須逐字核對」，不管「同型張力要比照先例」，本條補其缺口 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:f24e8d37c6131c583f5df2fe1f35c455ae87c6c0f47ab6d4488d1fc3eae944be -->
- 向使用者揭露某個選項的代價時寫得多具體，把它搬進 artifact 時就要多具體，不得概括成抽象詞——下游 stage 只讀 artifact，不會回頭比對問題檔。可執行檢查：選項說明裡點名的機制、檔名、以及牴觸的既有規則，逐一確認都出現在 artifact 本文。本輪實例：Q13 的選項說明點名「兩份 OpenRouter 客戶端與兩套串流事件語意」並點名牴觸 team.md 的「單一真實來源」規則，寫進 intent-statement.md 時被我稀釋成「兩份同類的執行機制」，等於把可執行資訊留在問題檔裡 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:fc73edfe5ae58e0e8b5932b1e868a1a3325d98bbc94a1cbac7c15d0195bc9e50 -->
- grep 命中一個名稱（資料表、函式、模組）不等於該物件是現行有效的——退役、棄用、不得使用這類狀態寫在區塊標題與 COMMENT 裡，不在名稱裡。可執行檢查：引用任何 schema 物件作為「某能力存在」的證據前，讀它所在的區塊標題與 COMMENT ON，並在記錄中寫明取得方式。本輪實例：我以 grep "CREATE TABLE.*cost" 取得 archive_diagram_cost 等三張表名就當成成本能力存在的證據，實際上 schema_rbac.sql 第 166 行的區塊標題逐字為「C1 Cost / FinOps tables — RETIRED」、三表的 COMMENT 逐字為「retired: … app must not read/write; drop after >=90d」，方向與結論相反；現行成本表是 estimate_sets 等六張，完全沒被引用到 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:intent-capture:ac1713c49d00ad683a4c13e0a5fb1de368814db1e62754e36522396a4c058ddf -->
- 因上游前提失效而重審可行性時，逐項對照的結果可能是**信心上調**，不是只會下調——新證據同樣可能把既有的假設升格為事實、讓既定的技術試探失去必要性。既有的 approval-handoff:rev1-c2 只涵蓋「不得因範圍變大就自動下修信心」，沒涵蓋反方向。可執行檢查：重審時對每一條 Assumption 與每一項技術試探各問一次「這一輪的新證據有沒有回答它？」，有就改標為事實或收窄範圍並寫明依據。本輪實例：A-3「LangGraph 與 Python 3.12 相容」因 langgraph==1.2.11 已釘選並在部署環境運行而由假設升為事實；F6 的 LangGraph 試探因 repo 已有可運行前例（cost_advice_agent.py 正是「LangGraph 圖＋SSE 對外」的形狀）而收窄為本 intent 獨有的部分。不做這一面，會把已被證明的事再驗一次 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:feasibility:3966ea4a875b3de475319d2f8828661c930bedc45cca49babad9d8941c49ef02 -->
- 確認某能力「已存在」之後，還要再查它的**交接層機制**才算查完——存在與可接是兩件事，而交接層的形狀會直接決定上游假設怎麼收。可執行檢查：對要編排或呼叫的既有能力，至少查三項——它的非同步性（是同步回傳還是排程 job）、它對外的事件或回應形狀（token 串流？狀態事件？一次回傳？）、它的授權掛在哪一層（FastAPI dependency？service 層自檢？無）。本輪實例：成本 agent 的「串流」實為每秒輪詢 DB 送 progress／completed／timeout／failed／heartbeat 的**狀態事件**而非 token 串流，上游據此寫的三個處置選項（逐字轉送／彙整後再送／並存）整個框架不成立；而授權掛在 require_story_action dependency 上，使「大腦要用 HTTP 還是同進程呼叫」成為授權能否被繞過的分水嶺，一題同時收斂了兩個上游假設 (learned 2026-09-21) <!-- cid:260920-orchestration-brain:feasibility:e36fda3a1af7b5fc829aabdcf17ae4ad93ecba2adaeae3060003520a43aeeee3 -->
- 排序理由要寫「這個位置由什麼決定」，而不是「因為它是 Should／Must」——分級與交付位置是兩件事，綁在一起寫會讓每一次分級變動都得重新論證整份排序。可執行檢查：backlog 的每一列排序理由，都要能指出它是被哪一條技術依賴、哪一項風險或哪一個避免重工的考量釘在該位置；指不出來就代表那個位置其實是分級的副產品。本輪實證：上一輪寫下「兩項 Should 排在最後，但不是因為它們是 Should——是因為技術依賴綁在序 2 與序 3 之後；即使升為 Must，順序也不會改變」，這一輪能力 10 真的由 Should 升為 Must，而位置確實不需要動，修訂只改了分級欄 (learned 2026-09-22) <!-- cid:260920-orchestration-brain:scope-definition:30627292d851b0c985b091c024f87975a416c4b10e745f5a6689bb40cf358cfa -->
- 取得 Consolidated Summary Confirmation 之前，所有產出必須已完全定稿——**包含問題檔本身的就地標註**。收據綁定的是整份問題檔的內容，之後任何改動（哪怕只是加一段 HTML 註解）都會讓它失效並回報 SUMMARY_CONTENT_STALE，必須整輪重取確認。正確順序為：(1) 產出與問題檔標註全部寫到定稿 → (2) `aidlc-log.ts decision --checkpoint summary-confirmation` 登記提示 → (3) 提問 → (4) `aidlc-log.ts answer` 記錄選擇並取得 Summary Authorization Id → (5) 之後才用 Write/Edit 工具重存每一份產出讓 hook 蓋戳記（以 python 直接寫檔不觸發 hook，會被判 SUMMARY_ARTIFACT_UNAUTHORIZED）。若問題檔也需要帶戳記的寫入，做法是以 Edit 加一個標記再以第二個 Edit 移除——兩次工具呼叫都觸發 hook，而最終內容與確認當下相同。本 session 在 feasibility 與 scope-definition 各踩一次，各多花一輪確認 (learned 2026-09-22) <!-- cid:260920-orchestration-brain:scope-definition:e8146aa4f0578412010722b0c7cac51162c89fd2e087daced6e81ec3b1db5c2f -->
