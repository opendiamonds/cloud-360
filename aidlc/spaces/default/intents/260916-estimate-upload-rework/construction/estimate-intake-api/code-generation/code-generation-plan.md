# Code Generation Plan — estimate-intake-api（U2）

> Unit: `estimate-intake-api` · kind: **service**  
> 上游：FD Q1–Q6=A、NFR Q1–Q6=A  
> 測試策略：Standard · methodology：**test-after**（見下方 Testing Contract）  
> **不含 SPA**（U8／U9）；SSE／建議正文屬 U7

## 前置事實（不重做）

| 項 | 狀態 |
|---|---|
| 模組 | `estimate_intake_router`／`service`／access／audit；掛 `/api/cost/v1` |
| 解析 | 呼叫 U1 `parse`／`validate` |
| 觸發建議 | BackgroundTasks enqueue；立即 201；失敗→Advice=failed＋audit |
| 授權 | 擁有者＋分享；diagram_id 純標籤 |
| 刪除 | 硬刪級聯 |
| RBAC | 更新 C1；移除 C1h／C1r／C1o／C1b；allow／deny 測 |
| 表 | EstimateSet／Estimate／LineItem／Share／AuditEvent；Advice 空殼 |
| OpenAPI | 同 PR 重產 `openapi.json`＋`gen:types` |
...
> service：Data model＝新表／ORM；API＝TestClient；Business＝access／upload 協調；Frontend e2e＝N/A（U8）。

## 追溯（計畫步驟 ↔ 需求）

| 步驟 | FR／BR／NFR | 說明 |
|---|---|---|
| Step 1 | FR6、BR2.11、NFR Q6 | ORM＋DDL 雙軌＋Advice 空殼 |
| Step 2 | FR7、BR2.5／2.10 | access＋RBAC seed |
| Step 3 | FR1、BR2.1–2.4、NFR3 | upload 協調＋驗證＋parse |
| Step 4 | C2、BR2.6–2.9 | router：sets／shares／advice GET／enqueue |
| Step 5 | FR8、NFR5 | audit＋log 遮罩 |
| Step 6 | team A／B、NFR | TestClient＋unittest；openapi；contract 綠 |
...
- [x] `estimate_access.py`（或等價）：owner／share 判斷；不查 collab 私有函式
- [x] `rbac_seed_data.py`：C1 語意更新；刪 C1h／C1r／C1o／C1b
- [x] allow／deny 測試（有 C1 vs 無 C1；owner vs 非分享者）
...
## Step 4 — HTTP router＋enqueue

- [x] `estimate_intake_router.py`：C2 paths（POST／GET list／GET detail／DELETE／shares GET／PUT／advice GET）
- [x] `main.py` 掛載 `/api/cost/v1`
- [x] 同 request enqueue hook；失敗→Advice failed＋audit；仍 201
- [x] list `page_size` ≤ 50

## Step 5 — Audit／obs

- [x] upload／share_replace／delete（日誌）／advice_enqueue／advice_enqueue_failed
- [x] 日誌禁金額／raw／檔案 bytes

## Step 6 — 測試、OpenAPI、交付物

- [x] `backend/tests/test_estimate_intake_api.py`（TestClient；≥5–8 案）
- [x] `dump_openapi.py`＋`npm run gen:types`；`validate_repo_contract`／`validate_env_contract` 綠
- [x] source-manifest／code-summary／traceability
- [x] **不**手寫 TCMS；**不**改 CostPage（U8）

## 不做

- SPA／SSE／建議正文／查價強化
- Redis／Celery／Alembic／APM
- 重建舊 diagrams API
