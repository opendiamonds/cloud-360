# Code Generation Plan — cost-advice-agent（U7）

> Unit: `cost-advice-agent` · kind: **service** · test-after · Standard
...
| 步驟 | FR／BR／NFR |
|---|---|
| 1 Orchestrator＋worker | BR7.5–7.7、7.10、NFR-SC.1 |
| 2 Agent＋U6／可選 U5 | BR7.1–7.4、7.9、NFR1.2 |
| 3 SSE router＋OpenAPI | C3、BR7.6–7.8、Q6 |
| 4 Wire enqueue＋main | FD Q1 |
| 5 Tests＋deliverables | NFR7.1、NFR9.1 |

## Step 1 — Orchestrator

- [x] `backend/cost/advice_orchestrator.py`：ThreadPoolExecutor(2)、enqueue、timeout 清理、寫 Advice
- [x] 替換 `estimate_intake_service.enqueue_advice_job`
...
- [x] `backend/cost/advice_stream_router.py`：`GET /sets/{id}/advice/stream`；heartbeat 15s；授權 U2
- [x] `main.py` 掛載；更新 openapi／types
...
- [x] `backend/tests/test_cost_advice_agent.py`：enqueue 去重、逾時 failed、SSE 授權、密鑰不洩、mock agent
- [x] 既有 intake 測試仍綠
