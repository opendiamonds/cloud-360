# Code Generation Plan — langgraph-runtime（U6）
...
| 項 | 狀態 |
|---|---|
| 模組路徑 | `backend/services/langgraph_runtime.py` |
| 公開 API | `invoke_graph → InvokeOutcome`；`stream_graph`／`astream_graph`；OpenRouter 客戶建構 |
| 逾時／重試 | 預設 60s；本 unit 不自動重試 |
| 預設模型 | `google/gemini-3.7-flash` |
| 缺金鑰 | `RuntimeAuthError`（變數名、不含值） |
| Tech | **`langgraph==1.2.11`**、**`langchain-openai==1.6.2`**；保留 `claude-agent-sdk` |
| 圖節點 | **不含**（U7） |
...
> library：Data model／Repository／API／Frontend 層標為 N/A；重點在 Business logic 與單元測試／smoke。

## 追溯（計畫步驟 ↔ 需求）

| 步驟 | FR／NFR／BR | 說明 |
|---|---|---|
| Step 1 | tech-stack Q4／Q3 | 釘 langgraph／langchain-openai；確認 claude-agent-sdk 仍在 |
| Step 2 | FR10.2、BR10.2、BR10.4、NFR9.1–9.2 | OpenRouter 客戶＋逾時＋缺金鑰錯誤 |
| Step 3 | FR10.1、BR10.1、BR10.5、BR10.8–10.9 | invoke／stream／astream |
| Step 4 | BR10.6、NFR7.1 | unittest mock＋smoke 腳本 |
| Step 5 | C5、NFR9.1 | contract／unittest 綠；交付物 |

---

## Step 1 — 依賴（Environment）

- [x] `backend/requirements.txt` 新增 `langgraph==1.2.11`、`langchain-openai==1.6.2`
- [x] 確認 `claude-agent-sdk` 列仍存在（FR10.3）
- [x] 本機可 `import langgraph`／`langchain_openai`

## Step 2 — OpenRouter 客戶（Business logic）

- [x] `openrouter_chat_model(...)`（或等價）：讀 `OPENROUTER_API_KEY`；`base_url=https://openrouter.ai/api/v1`；預設 model／timeout=60s
- [x] 缺金鑰 → `RuntimeAuthError`（NFR9.1／BR10.4）
- [x] **不**改 `llm_provider.py` 語意

## Step 3 — 圖執行輔助（Business logic）

- [x] `invoke_graph(graph, state, *, config?) -> InvokeOutcome`
- [x] `stream_graph(...) -> Iterator[StreamEvent]`（sync／smoke）
- [x] `astream_graph(...) -> AsyncIterator[StreamEvent]`（async／U7 SSE）
- [x] 不內建成本建議節點或提示詞（BR10.5）

## Step 4 — 測試與 smoke（test-after）

見 `unit-test-instructions.md`。摘要：

- [x] `backend/tests/test_langgraph_runtime.py`（mock：成功、缺金鑰、逾時形狀、金鑰不外洩）
- [x] `scripts/smoke_langgraph_openrouter.py`（有金鑰時選跑；CI 不強制）
- [x] 指令：`cd backend && PYTHONPATH=. python3 -m unittest tests.test_langgraph_runtime -v`
...
- 成本建議圖節點／提示詞／三類建議（U7）
- 查價（U5）、上傳 API（U2）
- 移除 `claude-agent-sdk`／改 `llm_provider`
- TCMS 手寫（Q6=A）
- CI 強制真實 OpenRouter 呼叫
