# Code Generation Plan — advice-presentation-ui（U9）
...
## Step 1 — EstimateAdvicePanel
- [x] `components/cost/EstimateAdvicePanel.tsx`：三態、三子標題、免責、testid=`advice-slot`

## Step 2 — SSE hook
- [x] fetch＋JWT＋ReadableStream 解析；AbortController；終態改 GET `/advice`

## Step 3 — Wire CostPage
- [x] 以 `EstimateAdvicePanel` 取代佔位

## Step 4 — e2e
- [x] 擴充／新增 Playwright：上傳後建議區進入 pending 或 completed／failed 可辨
