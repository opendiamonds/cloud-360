...
**第⑥類拆成四個單元而非一個**：四支 workflow 的失敗模式彼此不同（事件觸發 vs 排程產報告 vs 開 PR 防迴圈 vs CI 紅綠），合併會讓單一單元的完成判準同時指涉四種情境。這是 11 個單元略高於 [Q2=A] 的 8–10 的唯一原因，如實記載而非硬併。
...
| **不**擁有 | **[US:S-1 AC 7]（回寫 commit 不得取消既有 `ci.yml` run）歸 U-10**。理由：讓那件事為真的機制是 `ci.yml` 的 `paths-ignore`，不是本單元的回寫行為。若兩處都掛，U-4 需要 U-10 才驗得完、U-10 需要 U-4 才有 commit 可測——**依賴圖會出現環**，而 edge block 必須無環 |
| 複雜度 | **M** |
...
| --- | --- |
| `kind` | `service` |
| 擁有 | [ad:S-B] 與 [ad:C-7 `reconciler`]——兩者是同一個東西的兩面，拆開後都無法獨立驗收；每日排程、處理量上限、`ReconcileReport` 的九個欄位、一致率、五份清單、延遲量測 |
...
| 擁有 | `ci.yml` 的 `paths-ignore` 或等價手段（[US:S-1 AC 7]）；高成本 `on: pull_request` workflow（至少 `ui-regression`）對反向 PR 的排除（[US:S-6 AC 7]） |
| 交付 | 對 `.github/workflows/` 下**既有**檔案的修改 |
| 驗證方式 | 建置與觸發設定（觀察觸發是否發生，非執行期行為） |
| 完成判準 | 回寫 commit 推送後，既有 `ci.yml` run 未被取消且未新增一輪；反向 PR 開啟後 `ui-regression` 未對其執行 |
| 複雜度 | **S** |
| 實作註記 | **這是對既有檔案的修改，不是新機制。**[ad:component-dependency.md] 已確立其必要性、未指定手段——手段留 construction。`ci.yml` 的檔名是 load-bearing（在 `REQUIRED_FILES` 內，改名會讓 contract 紅燈） |
