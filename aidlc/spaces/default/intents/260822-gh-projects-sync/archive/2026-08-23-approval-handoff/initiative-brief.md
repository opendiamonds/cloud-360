...
- **scope-document** 與 **intent-backlog**（`../scope-definition/`）
...
**判定：Conditional GO** [Q2]。技術路徑存在且有官方支援（框架對看板更新提供受管的安全輸出型別，可依欄位名稱設定單選欄位），但建立在五個前提上，其中三項尚未滿足。

進入 INCEPTION 時仍未解、且已經人工確認接受的五項 [Q1]：
...
| U-5 | 十項全 Must 且宣告一次做完，與短生命週期分支實務在 deploy-on-merge 下相交 | delivery-planning |
...
**In Scope**：十項能力 CAP-1～CAP-10，全部列為 Must——綁定建立、推送觸發同步、PR 觸發同步（優先）、排程對帳、失敗通報、寫入前回讀、細粒度進展外置、README 指路文字、憑證可行性實測、驗證層。

**Won't Have**：反向同步、跨 repo 支援、自動關閉 issue、既有 71 個項目的一次性對正。
...
**建議 Conditional GO，進入 INCEPTION** [Q2]。

理由：INCEPTION 的全部工作是設計與規劃（reverse-engineering、requirements-analysis、user-stories、application-design、units-generation、delivery-planning），**沒有任何一站需要憑證即可進行**。未完成的三項前置依賴阻擋的是 CONSTRUCTION 的實際寫入，不是設計。在 phase 邊界停等會讓不需要憑證的工作停擺。

保留 conditional 字樣的理由：P-1／P-3／P-4 確實仍未完成，宣告 Full GO 會與事實不符。
...
- U-1～U-5 五項均已由 [Q1] 人工確認接受，但接受不等於解決；各自的落點見上表。 [assumption]
