...
| D-23 | 十項能力 CAP-1～CAP-10 全部列為 Must（**Revision 1 增為十一項，見 D-34**） | Q2＋Q2b |
| D-24 | 排序偏好 risk-first，但憑證實測為 Must 且**不構成交付批次** | Q3 |
| D-25 | 細粒度進展的落點：看板自訂欄位 | Q4 |
| D-26 | Won't Have 四項：反向同步、跨 repo 支援、自動關閉 issue、既有 71 項的一次性對正（**Revision 1 移出反向同步，減為三項，見 D-34**） | Q5＋Q8 |
| D-27 | 無硬時程 | Q6 |
...
| D-37 | Revision 1 新增的 U-6（CAP-11 未經本 intent feasibility 評估）與 U-7（PU-10 驗證落點未定）確認接受，指派 application-design | Q4 |

## ADR-0013（2026-08-23，修訂 ADR-0012）

這四項不是問答產物，而是發現 ADR-0012 未被引用後的架構級裁決，以 ADR 形式定錨。

| # | 決策 | 對應 |
| --- | --- | --- |
| D-33 | **映射層級維持 intent → Project #16 的一則 issue**，不採 ADR-0012 的 intent → 一整個 Project。`story → Issue` 記為未來方向而非否決 | ADR-0013 決定 1 |
| D-34 | **反向同步納入範圍**，推翻 D-26 把它列入 Won't Have 的決定。採納 ADR-0012 的論證：純鏡像會讓拖動的卡片被彈回原位，比沒有同步更糟 | ADR-0013 決定 2 |
| D-35 | **承載形式為 gh-aw safe-outputs，不建 `scripts/`**。ADR-0012 第 5 點的提權論證因 `update-project` 存在而失效 | ADR-0013 決定 3 |
| D-36 | **階段順序重排**：本次直接做 intent 層 Projects 同步，ADR-0012 的階段 1（story → Issue）不構成前置 | ADR-0013 決定 4 |
...
| E-3 | **ADR-0012 未被引用** | reverse-engineering 開始前發現 ADR-0012（Accepted 2026-08-16）涵蓋本 intent 主題，而 IDEATION 四站全程未引用。成因是 feasibility 的唯讀查證涵蓋 workflows、gh-aw 文件、GitHub API 與 repo 現況，唯獨未查 `decisions/`。比對出四處衝突。 |
| E-4 | **ADR-0013 開立** | 經使用者裁決，修訂 ADR-0012 的第 1 點（映射層級維持 intent→issue）、第 5 點（承載改為 gh-aw safe-outputs，其提權論證因 `update-project` 存在而失效）與階段表（本次不需先做階段 1）；並採納其對反向同步的論證。ADR-0012 的 Status 已加註修訂指向。 |
| E-5 | **回跳 scope-definition（Modify）** | 反向同步入範圍構成 scope 擴充，依 `project.md` 回跳修訂並重走 gate。重置 3 個 stage，舊 artifact 歸檔於 `archive/2026-08-23-*`。新增 CAP-11／PU-10／P-5，W-1 移出 Won't Have（以刪除線保留原紀錄）。 |
...
D-26（Won't Have 四項）經 ADR-0013 修訂：反向同步移出排除清單，Won't Have 減為三項。原決定保留於 scope-document 的刪除線紀錄中，不改寫。

新增 D-33～D-36（ADR-0013 的四項架構裁決）與 D-37（Q4 的未解項確認），以及非問答決策 E-3～E-5。決策總數由 32 項增為 **37 項**，非問答決策由 2 項增為 **5 項**。
